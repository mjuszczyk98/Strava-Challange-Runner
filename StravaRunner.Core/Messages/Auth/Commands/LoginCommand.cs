...
public record LoginCommand(string UserEmail) : IRequest;
...
{
    private const string HttpLocalhostAuthorize = "http://localhost:5000/auth/authorize";
    
    private readonly IMongoDatabase _database = database;
    private readonly IEncryptionService _encryptionService = encryptionService;
    private readonly IEmailService _emailService = emailService;
    private readonly StravaAuthSettings _authSettings = authSettings.Value;

    public async Task Handle(LoginCommand request, CancellationToken cancellationToken)
    {
        var usersCollection = _database.GetDbCollection<User>();
        ...
        var authorizeUrl = user.IsVerified ? HttpLocalhostAuthorize : GetStravaAuthorizeUrl(authorizeToken);
